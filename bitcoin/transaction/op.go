...
/*
OP_CHECKMULTISIG, structure of the evaluate stack:
1, number of public keys (2pubkeys)
2, pubkey2
3, pubkey1
4. number of signatures (1 signatures)
5. signature
6. []
*/
...
func (b *BitcoinOpCode) opCheckMultiSig(zBin []byte) bool {
	...
	//read the top element to get the number of public key
	...
	for i := 0; i < sigCounts; i++ {
		...
		//remove the last byte, it is hash type
...
func (b *BitcoinOpCode) opP2sh() bool {
	...
	b.RemoveCmd()
	/*
		the top element of stack is content of reddemscript, cache it then
		do hash160 on it
	*/
	...
	//need to parse the reddemscript and execute its command
	...
	redeemScritSig := NewScriptSig(bufio.NewReader(scriptReader))
	//bug here
	b.cmds = append(b.cmds, redeemScritSig.bitcoinOpCode.cmds...)
	return true
